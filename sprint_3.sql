...
#Para poder cambiar el dato:
UPDATE credit_card SET IBAN = "TR323456312213576817699999"
WHERE id = "Ccu-2938";

#confirmamos que el dato se haya modificado:
SELECT iban 
FROM transactions.credit_card
WHERE id = "Ccu-2938";
...
#para confirmar que se haya eliminado, vuelvo a pedir el contenido de la tabla
SELECT *
FROM credit_card;

##NIVEL 2
#ejercicio 1:
#Primero prefiero consultar por el registro a eliminar:
SELECT *
FROM transaction
...
#confirmo que se haya eliminado:
SELECT *
FROM transaction
WHERE id = '02C6201E-D90A-1859-B4EE-88D2986D3B02';
...
#en el pdf se ve la vista de marketing creada

#ejercicio 3:
#ver el pdf porque lo que he realizado se ve en la vista VIEW.

##NIVEL 3:
#ejercicio 1:
#Los comandos que habría que ejecutar son los siguientes:
...
#ejercicio 2:
SELECT transaction.id AS transaction_id, user.name AS user_name, user.surname AS user_surname, credit_card.iban, company.company_name
...
GROUP BY transaction.id, user.id, user.name, user.surname, credit_card.iban, company.company_name
